...
_GET_RANKS_EXACT = frozenset(['الادمنيه', 'المالكين', 'المالكين الاساسيين', 'المدراء', 'المشرفين', 'المكتومين', 'المميزين', 'قائمه Dev', 'قائمه MY'])
_GET_RANKS_PREFIXES = ()

@Client.on_message(filters.text & filters.group, group=12)
def getRanksHandler(c,m):
   routed_text = normalize_message_text(m)
   if not matches_command(routed_text, _GET_RANKS_EXACT, _GET_RANKS_PREFIXES):
      return False
   k = r.get(f'{Dev_Zaid}:botkey')
   channel = r.get(f'{Dev_Zaid}:BotChannel') if r.get(f'{Dev_Zaid}:BotChannel') else 'yqyqy66'
   return get_ranks_func(c,m,k,channel)
