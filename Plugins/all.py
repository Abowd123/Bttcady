...
async def on_zbi(c: Client, m: Message):
    ...
    name = r.get(f"{Dev_Zaid}:BotName") or "ليو"
    text = m.text
...
def guardLocksResponse(c, m):
    ...
    return guardResponseFunction(c, m, k, channel)
...
def guardLocksResponse2(c, m):
    ...
    return guardResponseFunction2(c, m, k, channel)
...
def guardCommandsHandler(c, m):
    ...
    return guardCommands(c, m, k, channel)
    ...
    if text.startswith("انطق "):
        if not r.get(f"{m.chat.id}:disableSay:{Dev_Zaid}"):
         ...
         """
            ...
            subprocess.run(
                [
                    "ffmpeg", "-y", "-i", f"zaid{id}.mp3", "-ac", "1",
                    "-strict", "-2", "-codec:a", "libopus", "-b:a", "128k",
                    "-vbr", "off", "-ar", "24000", f"zaid{id}.ogg",
                ],
                check=True,
                timeout=120,
            )
...
def CallbackQueryHandler(c, m):
    ...
    return CallbackQueryResponse(c, m, channel)
