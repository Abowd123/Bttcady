...
async def open_hms(c: Client, m: Message):
   ...
   if not wsdb.get(f"hms-{id}"):
      ...
      to = data["to"]
      if m.from_user.id != to and m.from_user.id != data["from"] and m.from_user.id != sudo_id and m.from_user.id != sudo_id:
...
def sudosCommandsHandler(c,m):
    k = r.get(f'{Dev_Zaid}:botkey')
    channel = r.get(f'{Dev_Zaid}:BotChannel') if r.get(f'{Dev_Zaid}:BotChannel') else 'yqyqy66'
    return SudosCommandsFunc(c,m,k,r,channel)
...
def SudosCommandsFunc(c,m,k,r,channel):
   ...
   if r.get(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Zaid}'):
      r.delete(f'{m.chat.id}:setBotowmer:{m.from_user.id}{Dev_Zaid}')
      return m.reply(quote=True, text=f'{k} نقل الملكية من داخل تيليجرام معطل؛ غيّر OWNER_ID في بيئة التشغيل')
   ...
   if text == 'تغيير المطور الاساسي':
     if devp_pls(m.from_user.id,m.chat.id):
        return m.reply(quote=True, text=f'{k} غيّر OWNER_ID في بيئة التشغيل ثم أعد تشغيل الخدمة')

   if text == 'تحديث':
     if devp_pls(m.from_user.id,m.chat.id):
        return m.reply(quote=True, text=f'{k} التحديث وإعادة التشغيل من تيليجرام معطلان أمنيًا')
