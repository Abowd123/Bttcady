...
from helpers.message_context import matches_command, normalize_message_text

_PROMOTE_EXACT = frozenset(['تعطيل الرفع', 'تفعيل الرفع', 'رفع Dev', 'رفع MY', 'رفع ادمن', 'رفع مالك', 'رفع مالك اساسي', 'رفع مدير', 'رفع مميز'])
_PROMOTE_PREFIXES = ('رفع Dev ', 'رفع MY ', 'رفع ادمن ', 'رفع مالك ', 'رفع مالك اساسي ', 'رفع مدير ', 'رفع مميز ')
_DEMOTE_EXACT = frozenset(['تنزيل Dev', 'تنزيل MY', 'تنزيل ادمن', 'تنزيل الكل', 'تنزيل مالك', 'تنزيل مالك اساسي', 'تنزيل مدير', 'تنزيل مميز'])
_DEMOTE_PREFIXES = ('تنزيل Dev ', 'تنزيل MY ', 'تنزيل ادمن ', 'تنزيل الكل ', 'تنزيل مالك ', 'تنزيل مالك اساسي ', 'تنزيل مدير ', 'تنزيل مميز ')


@Client.on_message(filters.text & filters.group, group=7)
def ranksCommandsHandler(c,m):
   routed_text = normalize_message_text(m)
   if not matches_command(routed_text, _PROMOTE_EXACT, _PROMOTE_PREFIXES):
      return False
   k = r.get(f'{Dev_Zaid}:botkey')
   return ranks_reply_promote(c,m,k)
...
def ranksCommandsHandlerDemote(c,m):
   routed_text = normalize_message_text(m)
   if not matches_command(routed_text, _DEMOTE_EXACT, _DEMOTE_PREFIXES):
      return False
   k = r.get(f'{Dev_Zaid}:botkey')
   return ranks_reply_demote(c,m,k)
...
def ranks_reply_demote(c,m,k):
    ...
    if text.startswith('تنزيل الكل '):
       ...
       if devp_pls(m.from_user.id,m.chat.id):
          ...
          if not rank == 'عضو' and not id in [sudo_id]:
              m.reply(f'「 {mention} 」\n{k} نزلته من {rank} \n☆')
              r.delete(f'{id}:rankDEV2:{Dev_Zaid}')
              r.srem(f'{Dev_Zaid}DEV2', id)
              r.delete(f'{id}:rankDEV:{Dev_Zaid}')
       ...
       if dev2_pls(m.from_user.id, m.chat.id):
          rank = get_rank(id,cid)
          if not rank == 'عضو' and not id == int(r.get(f'{Dev_Zaid}botowner')) and not id in [sudo_id]:
              m.reply(f'「 {mention} 」\n{k} نزلته من {rank} \n☆')
              r.delete(f'{id}:rankDEV:{Dev_Zaid}')
              r.srem(f'{Dev_Zaid}DEV', id)
              r.delete(f'{cid}:rankGOWNER:{id}{Dev_Zaid}')
              ...
              r.srem(f'{cid}:listPRE:{Dev_Zaid}', id)
              return
          if id in [sudo_id] or id == int(r.get(f'{Dev_Zaid}botowner')):
              return m.reply(f'{k} رتبته اعلى منك')
          else:
              return m.reply(f'{k} ماله رتبة')

       if not rank == 'عضو' and not id == int(r.get(f'{Dev_Zaid}botowner')) and not id in [sudo_id] and not r.get(
           ...
           r.srem(f'{cid}:listPRE:{Dev_Zaid}', id)
           return
       if id in [sudo_id] or id == int(r.get(f'{Dev_Zaid}botowner')) or not r.get(
               f'{id}:rankDEV2:{Dev_Zaid}'):
           return m.reply(f'{k} رتبته اعلى منك')
       else:
           return m.reply(f'{k} ماله رتبة')
    ...
    if text == 'تنزيل الكل' and m.reply_to_message and m.reply_to_message.from_user:
       ...
       if gowner_pls(m.from_user.id, m.chat.id):
           ...
           if not rank == 'عضو' and not id == int(r.get(f'{Dev_Zaid}botowner')) and not id in [
               sudo_id] and not r.get(f'{id}:rankDEV2:{Dev_Zaid}') and not r.get(f'{id}:rankDEV:{Dev_Zaid}'):
               m.reply(f'「 {mention} 」\n{k} نزلته من {rank} \n☆')
               r.delete(f'{cid}:rankOWNER:{id}{Dev_Zaid}')
               r.srem(f'{cid}:listOWNER:{Dev_Zaid}', id)
               r.delete(f'{cid}:rankMOD:{id}{Dev_Zaid}')
       ...
       if mod_pls(m.from_user.id, m.chat.id):
           rank = get_rank(id, cid)
           if not rank == 'عضو' and not id == int(r.get(f'{Dev_Zaid}botowner')) and not id in [
               sudo_id] and not r.get(f'{id}:rankDEV2:{Dev_Zaid}') and not r.get(
       ...
       if admin_pls(m.from_user.id, m.chat.id):
           ...
           if not rank == 'عضو' and not id == int(r.get(f'{Dev_Zaid}botowner')) and not id in [
               ...
               r.srem(f'{cid}:listPRE:{Dev_Zaid}', id)
               return
           if id in [sudo_id] or id == int(r.get(f'{Dev_Zaid}botowner')) or r.get(
                   f'{id}:rankDEV2:{Dev_Zaid}') or r.get(f'{id}:rankDEV:{Dev_Zaid}') or r.get(
